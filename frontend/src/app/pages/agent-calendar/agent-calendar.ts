...
@Component({
  ...
  imports: [NgFor, NgIf],
  templateUrl: './agent-calendar.html',
  styleUrl: './agent-calendar.css'
})
export class AgentCalendar implements OnInit {
  ...
  loadMeetings(): void {
    ...
    this.http.get<Meeting[]>(`${this.meetingsApiUrl}/agent/${this.user.id}`).subscribe({
      ...
      error: (error) => {
        console.error('Błąd pobierania kalendarza agenta:', error);
        alert('Nie udało się pobrać Twojego kalendarza.');
      }
    });
  }
