...
export class AdminReport implements OnInit {
  ...
  buildReportInsights(report: AdminOverviewReport): string[] {
    ...
    const agentsWithoutMeetings = report.meetingsByAgent.filter(item => item.meetingsCount === 0).length;
    const agentsWithoutWorkTime = report.workTimeByAgent.filter(item => item.entriesCount === 0).length;
    const mostLoadedAgent = report.workTimeByAgent.find(item => item.totalHours > 0);
    const mostScheduledAgent = report.meetingsByAgent.find(item => item.meetingsCount > 0);
  ...
  private getAgentPlural(value: number): string {
    return value === 1 ? 'agent' : 'agentów';
  }
